...
extern volatile int32_t crossingPeriod2;
extern volatile int32_t adcCrossingPeriod;
extern uint32_t nextCrossingDetect;
extern volatile uint32_t GoodDetected;
extern volatile uint32_t BadDetected;
extern float duty;
extern uint32_t Period_PWM;
extern float rpm;
volatile uint32_t runCount;

uint8_t runDuty(float rduty);
...
void cliFuncStatus(void *cmd, char *cmdLine) {
    ...
    /*
    serialPrintF( formatInt, "A0", adcRawData[0]);
    serialPrintF( formatInt, "A1", adcRawData[1]);
    serialPrintF( formatInt, "A2", adcRawData[2]);
    serialPrintF( formatInt, "A3", adcRawData[3]);
    */
//    serialPrintF( formatInt, "minLooptime", minCycles);
    sprintf(tempBuf, formatFloat, "RPM", rpm);
    serialPrint(tempBuf);
    ...
    sprintf(tempBuf, formatInt, "BAD DETECTS", BadDetected);
    serialPrint(tempBuf);

    sprintf(tempBuf, formatFloat, "FET DUTY", duty*100);
    serialPrint(tempBuf);

  //  serialPrintF( formatInt, "Gooddetected", GoodDetected);

   // serialPrintF( formatInt, "Baddetected", BadDetected);
   // serialPrintF( formatInt, "Looptime", cycles);
   // serialPrintF( formatFloat, "PERCENT IDLE", idlePercent);
}
...
uint8_t runDuty(float rduty) {
    ...
    if (rduty >= 0.0f || rduty <= 100.0f) {
 //   runMode = OPEN_LOOP;
    fetSetDutyCycle((uint16_t)(Period_PWM*rduty*0.01f));
    duty=rduty*0.01f;
    ret = 1;
    }
...
void fetSetDutyCycle(int32_t requestedDutyCycle) {
    ...
    if (requestedDutyCycle > Period_PWM)
    requestedDutyCycle = Period_PWM;
    else if (requestedDutyCycle < 0)
    requestedDutyCycle = 0;

    LL_TIM_OC_SetCompareCH1(TIM4,requestedDutyCycle);
    LL_TIM_OC_SetCompareCH2(TIM4,requestedDutyCycle);
    LL_TIM_OC_SetCompareCH3(TIM4,requestedDutyCycle);
